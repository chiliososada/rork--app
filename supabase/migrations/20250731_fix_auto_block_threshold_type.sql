-- コンテンツモデレーションシステムの修正
-- auto_block_threshold のデータ型エラーを修正

-- moderate_content 関数を修正して数値型の設定値を適切に処理
...
DECLARE
  ...
  block_threshold DECIMAL DEFAULT 8.0; -- DECIMALに変更
...
BEGIN
  ...
  -- auto_block_threshold を DECIMAL として処理
  ...
  -- 最終的なアクションを決定（block_threshold は DECIMAL）
  IF is_auto_blocked OR total_severity::DECIMAL >= block_threshold THEN
